...
class Html extends TagLib{
    ...
    protected $tags   =  array(
		...
		'imgUpload' => array('attr'=>'id,name,style,datasource,repetition','close'=>0)
    );
    ...
    public function _list($tag) {
		...
		$parseStr	= "<!-- Think 系统列表组件开始 -->\n";
        $parseStr  .= '<table id="'.$id.'" class="'.$style.'" cellpadding=0 cellspacing=0 >';
        $parseStr  .= '<tr><td colspan="'.$colNum.'" class="topTd" style="padding:2px 10px;" ><a href="javascript:;" style="float:right;" onclick="window.location.reload()" class="icon-refresh"></a></td></tr>';
        $parseStr  .= '<tr class="row" >';
        //列表需要显示的字段
        $fields = array();
        foreach($show as $val) {
    ...
    public function _imgUpload($tag)
    {
		...
		$datasource = $tag['datasource'];               //列表显示的数据源VoList名称
        $style      = $tag['style'];                    //样式名
        $rep 		= !empty($tag['repetition'])?$tag['repetition']:0;//是否是多图上传
		if($style){
			$parseStr = '<div class="'.$style.'">';
		}else{
			$parseStr = '<div style="float:left;position: relative;">';
		}
		$parseStr .= '<input type="file" id="'.$id.'" name="'.$name.'" repetition="'.$rep.'" />';
		$parseStr .= '</div>';
		$parseStr .= '<div style="float:left;margin-left:20px;height:50px;line-height:45px" name="'.$name.'_show">';
		$parseStr .= '<volist name="'.$datasource.'" id="vo">';
		$parseStr .= '<img style="margin-left:10px;" src="__UPLOADS__/{$vo.file|addslashes}" width="50" height="45" onclick="delimage(this,\'{$vo.id|addslashes}\')"/>';
		$parseStr .= '</volist></div>';
