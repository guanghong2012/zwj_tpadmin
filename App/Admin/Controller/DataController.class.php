...
class DataController extends backendController{
    ...
    function index() {
        //\Think\Hook::add('test','Common\\Behaviors\\InitHookBehavior');
        //echo \Think\Hook::listen('test');die;
        ...
        $startsize = $pagesize*($nowPage-1);//开始条数
        $endsize = $startsize + $pagesize;//结束条数
    ...
    public function optimize($tables = null){
        ...
        if($tables) {
            ...
            if(is_array($tables)){
                ...
                $list = $this->model->execute("OPTIMIZE TABLE `{$table}`");

                if($list){
                    save_log("数据表'".implode(",", $tables)."'优化完成！",1);
                    IS_AJAX &&  $this->ajaxReturn(1, '数据表优化完成！', U("data/tablist"), '');
                    $this->success("数据表优化完成！");
                } else {
                    save_log("数据表'".implode(",", $tables)."'优化出错！",1);
                    IS_AJAX &&  $this->ajaxReturn(0, '数据表出错请重试！', U("data/tablist"), '');
                    $this->error("数据表优化出错请重试！");
                }
            } else {
                $list = $this->model->execute("OPTIMIZE TABLE `{$tables}`");
                if($list){
                    save_log("数据表'{$tables}'优化完成！",1);
                    IS_AJAX &&  $this->ajaxReturn(1, '数据表优化完成！', U("data/tablist"), '');
                    $this->success("数据表'{$tables}'优化完成！");
                } else {
                    save_log("数据表'{$tables}'优化出错！",1);
                    IS_AJAX &&  $this->ajaxReturn(0, '数据表优化出错！', U("data/tablist"), '');
                    $this->error("数据表'{$tables}'优化出错请重试！");
                }
            }
        } else {
            IS_AJAX &&  $this->ajaxReturn(0, '请指定要优化的表！', U("data/tablist"), '');
            $this->error("请指定要优化的表！");
        }
    }
    /**
    ...
    public function repair($tables = null){
        ...
        if($tables) {
            ...
            if(is_array($tables)){
                $table = implode('`,`', $tables);
                $list = $this->model->execute("REPAIR TABLE `{$table}`");

                if($list){
                    save_log("数据表'".implode(",", $tables)."'修复完成！",1);
                    IS_AJAX &&  $this->ajaxReturn(1, '数据表修复完成！', U("data/tablist"), '');
                    $this->success("数据表修复完成！");
                } else {
                    save_log("数据表'".implode(",", $tables)."'修复出错！",1);
                    IS_AJAX &&  $this->ajaxReturn(0, '数据表修复出错请重试！', U("data/tablist"), '');
                    $this->error("数据表修复出错请重试！");
                }
            } else {
                $list = $this->model->execute("REPAIR TABLE `{$tables}`");
                if($list){
                    save_log("数据表'{$tables}'修复完成！",1);
                    IS_AJAX &&  $this->ajaxReturn(1, '数据表修复完成！', U("data/tablist"), '');
                    $this->success("数据表'{$tables}'修复完成！");
                } else {
                    save_log("数据表'{$tables}'修复出错！",1);
                    IS_AJAX &&  $this->ajaxReturn(0, '数据表修复出错请重试！！', U("data/tablist"), '');
    ...
    function backall() {
        ...
        if ($this->backup($tables)) {
            save_log("备份整个数据库！",1);
            IS_AJAX &&  $this->ajaxReturn(1, '数据库备份成功！', U("data/index"), '');
            $this->success('数据库备份成功！',U("data/index"));
        } else {
            save_log("备份整个数据库！",0);
            IS_AJAX &&  $this->ajaxReturn(0, '数据库备份失败！', U("data/index"), '');
            $this->error('数据库备份失败！');
        }
    }
    ...
    function backtables() {
        ...
        $this->explain = str_replace(C("DB_PREFIX"),"",$tab);
        ...
        if (is_array($tab))
            ...
            $tables[] = $tab;
        if ($this->backup($tables)) {
            save_log("备份".implode(",", $tables)."表！",1);
            IS_AJAX &&  $this->ajaxReturn(1, '数据库备份成功！', U("data/index"), '');
            $this->success('数据库备份成功！',U("data/index"));
        } else {
            save_log("备份".implode(",", $tables)."表！",0);
            IS_AJAX &&  $this->ajaxReturn(0, '数据库备份失败！');
            $this->error('数据库备份失败！');
        }
    }
    ...
    function recover() {
        ...
        if ($this->recover_file($_GET['file'])) {
            save_log("还原备份数据".$_GET['file']."！",1);
            $this->success('数据还原成功！', U("data/index"));
        } else {
            save_log("还原备份数据".$_GET['file']."！",0);
            $this->error('数据还原失败！');
        }
    }
    ...
    function deletebak() {
        if (unlink($this->config['path'] . $this->dir_sep . $_GET['file'])) {
            save_log("删除备份数据".$_GET['file']."！",1);
            $this->success('删除备份成功！', U("data/index"));
        } else {
            save_log("删除备份数据".$_GET['file']."！",0);
            $this->error('删除备份失败！');
        }
    }
    ...
    private function setFile() {
        ...
        $fileName = $this->trimPath($this->config['path'] . $this->dir_sep . $recognize . '_' . date('YmdHis') . '_' . $this->explain . '.sql');
